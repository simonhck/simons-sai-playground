...
import { getEdgeWebhookCompatTags } from 'src/lib/cache/edge-webhook-tag-compat';
...
export async function getSitecorePage(params: GetSitecorePageParams): Promise<Page | null> {
  ...
  for (const tag of [...tags, ...getEdgeWebhookCompatTags(page?.layout?.sitecore?.route, locale)]) {
    cacheTag(tag);
  }
